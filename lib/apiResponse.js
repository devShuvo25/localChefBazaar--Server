...
async function run(app) {
 ...
 app.get("/meals", async (req, res) => {
  try {
    ...
    try {
    const email = req.params.email;
    const { searchValue = "", sortValue = "all", fv = "all", fv2: priceFilter = "all" } = req.query;

    // Base filter with user email
    const filter = { userEmail: email };

    // Search filter
    if (searchValue) {
      filter.foodName = { $regex: searchValue.trim(), $options: "i" };
    }

    // Category filter
    if (fv && fv !== "all") {
      filter.orderStatus = { $regex: fv.trim(), $options: "i" };
    }
    ...
    if (sortValue === "asc") {
      ...
      sortOption = { orderTime: -1 };
    } else if (sortValue === "oldest") {
      sortOption = { orderTime: 1 };
    } else {
      sortOption = { orderTime: -1 }; // Default sort
    }
